...
if (!/^(localhost|127\.0\.0\.1)$/.test(location.hostname)) {
    ...
    document.addEventListener("input", (e) => {
        const t = e.target;
        if (!t || !t.matches?.(PROMPT_SELECTOR))
            return;
    ...
    function buildPromptSuggestions(text, scores) {
        ...
        const t = text.trim();
        if (t.length < 80)
            s.push("Add specifics: audience, domain, constraints, and examples.");
        if (!/[?.!]/.test(t))
            s.push("Ask as a clear question or add an objective.");
        ...
        if (!s.length)
            s.push("Nice prompt. You can refine with target, style and constraints if needed.");
        return s;
    }
}
